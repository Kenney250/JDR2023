...
pdf("Nolan-Endo-DotPlot.pdf", width = 40, height = 30)
...
#EMT Gene list from GSEA
#https://www.gsea-msigdb.org/gsea/msigdb/cards/HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION.html
EMT_FullList <- c("ABI3BP" , "ACTA2", "ADAM12", "ANPEP", "APLP1", "AREG", "BASP1", "BDNF", "BGN", "BMP1", 
                "CADM1", "CALD1", "CALU", "CAP2", "CAPG", "CCN1", "CCN2", "CD44", "CD59", "CDH11", "CDH2", 
                "CDH6", "COL11A1", "COL12A1", "COL16A1", "COL1A1", "COL1A2", "COL3A1", "COL4A1", "COL4A2", 
                "COL5A1", "COL5A2", "COL5A3", "COL6A2", "COL6A3", "COL7A1", "COL8A2", "COLGALT1", "COMP", "COPA", 
                "CRLF1", "CTHRC1", "CXCL1", "CXCL12", "CXCL6", "CXCL8", "DAB2", "DCN", "DKK1", "DPYSL3", "DST", 
                "ECM1", "ECM2", "EDIL3", "EFEMP2", "ELN", "EMP3", "ENO2", "FAP", "FAS", "FBLN1", "FBLN2", "FBLN5", 
                "FBN1", "FBN2", "FERMT2", "FGF2", "FLNA", "FMOD", "FN1", "FOXC2", "FSTL1", "FSTL3", "FUCA1", "FZD8", 
                "GADD45A", "GADD45B", "GAS1", "GEM", "GJA1", "GLIPR1", "GPC1", "GPX7", "GREM1", "HTRA1", "ID2", 
                "IGFBP2", "IGFBP3", "IGFBP4", "IL15", "IL32", "IL6", "INHBA", "ITGA2", "ITGA5", "ITGAV", "ITGB1", 
                "ITGB3", "ITGB5", "JUN", "LAMA1", "LAMA2", "LAMA3", "LAMC1", "LAMC2", "LGALS1", "LOX", "LOXL1", 
                LOXL2, LRP1, LRRC15, LUM, MAGEE1, MATN2, MATN3, MCM7, MEST, MFAP5, MGP, MMP1, MMP14, MMP2, MMP3, MSX1,MXRA5,MYL9,MYLK,NID2,NNMT,NOTCH2,NT5E,NTM,OXTR,P3H1,PCOLCE,PCOLCE2,PDGFRB,PDLIM4,PFN2,PLAUR,PLOD1,PLOD2,PLOD3,PMEPA1,PMP22,POSTN,PPIB,PRRX1,PRSS2,PTHLH,PTX3,PVR,QSOX1,RGS4,RHOB,SAT1,SCG2,SDC1,SDC4,SERPINE1,SERPINE2,SERPINH1,SFRP1,SFRP4,SGCB,SGCD,SGCG,SLC6A8,SLIT2,SLIT3,SNAI2,SNTB1,SPARC,SPOCK1,SPP1,TAGLN,TFPI2,TGFB1,TGFBI,TGFBR3,TGM2,THBS1,THBS2,THY1,TIMP1,TIMP3,TNC,TNFAIP3,TNFRSF11B,TNFRSF12A,TPM1,TPM2,TPM4,VCAM1,VCAN,VEGFA,VEGFC,VIM,WIPF1,
                WNT5A)
...
pdf("EndoMT-Endo-Inflammation-DotPlot.pdf", width = 40, height = 30)
...
pdf("EndoMT-Endo-EC_Gene-DotPlot.pdf", width = 40, height = 30)
...
pdf("EndoMT-Endo-Mesen_Gene-DotPlot.pdf", width = 40, height = 30)
...
pdf("EndoMT-Endo-TF_Gene-DotPlot.pdf", width = 40, height = 30)
...
pdf("EndoMT-Endo-Subset-DotPlot.pdf", width = 40, height = 30)
print(DotPlot(aSeuratIntegrated, features = EndoMT_subset, cols = c("lightblue", "red3"), 
...
newClusterIDs <- endo_lables_long_dot
names(newClusterIDs) <- levels(aSeuratEndo)
aSeuratEndo <- RenameIdents(aSeuratEndo, newClusterIDs)
levels(aSeuratEndo)

subClusterMarkers <- c(EmailedMarkers, "Kdr", "Hbb-bs", "Hba-a1")
pdf("ClusterMarkers_subset2.pdf", width = 40, height = 30)
print(DotPlot(aSeuratEndo, features = subClusterMarkers, cols = c("lightblue", "red3"), col.min = 0, dot.scale = 30) +
        theme(axis.title = element_text(size = 60), axis.text = element_text(size = 50), 
              legend.text = element_text(size = 40), legend.key.size = unit(3, 'cm'), 
              legend.title  = element_text(size = 40), legend.spacing = unit(3, 'cm')) +
        RotatedAxis())
...
EmailedMarkers<- c("Apoe", "Il6st", "Sema3g", "Clu", "Ccl21a", "Mmrn1", "Gpihbp1", "Rgcc")
...
levels(aSeuratEndo)
levels(aSeuratEndo) <- c("Mock_Artery", "Ligated_Artery", "Deligated_Artery", 
                         "Mock_Vein", "Ligated_Vein", "Deligated_Vein", 
                         "Mock_Capillary", "Ligated_Capillary", "Deligated_Capillary", 
                         "Mock_Lymphatic", "Ligated_Lymphatic", "Deligated_Lymphatic", 
                         "Mock_RBC", "Ligated_RBC", "Deligated_RBC")

levels(aSeuratEndo)

endoSubsetIdents <- c("Mock_Artery", "Ligated_Artery", "Deligated_Artery", 
                      "Mock_Vein", "Ligated_Vein", "Deligated_Vein", 
                      "Mock_Capillary", "Ligated_Capillary", "Deligated_Capillary", 
                      "Mock_Lymphatic", "Ligated_Lymphatic", "Deligated_Lymphatic")

pdf("EmailedMarkers_subset.pdf", width = 40, height = 30)
print(DotPlot(aSeuratEndo, features = EmailedMarkers, cols = c("lightblue", "red3"), col.min = 0, dot.scale = 30) +
                theme(axis.title = element_text(size = 60), axis.text = element_text(size = 50), 
                      legend.text = element_text(size = 40), legend.key.size = unit(3, 'cm'), 
                      legend.title  = element_text(size = 40), legend.spacing = unit(3, 'cm')) +
                RotatedAxis())
...
pdf("./Endo-markers/NolanMarkers_subset_short.pdf", width = 40, height = 30)
...
pdf("./Endo-markers/TFMarkers_subset_short.pdf", width = 40, height = 30)
...
pdf("./Endo-markers/MeseMarkers_subset_short.pdf", width = 40, height = 30)
...
Endolist_short <- c("Cdh5", "Kdr", "Pecam1", "Tie1", "Tek", "Flt4", "Vwf", "Nos3", "Tal1", "Fgfr1")
pdf("./Endo-markers/EndoMarkers_subset_short.pdf", width = 40, height = 30)
...
pdf("./Endo-markers/InflMarkers_subset_short.pdf", width = 40, height = 30)
...
ligand <- "H2-T23"
target <- "Immune"
aNATMISubset <- subset(NATMImockvligated, Sending.cluster == "Artery" & Ligand.symbol == ligand & Target.cluster == target)[,c(1, 2, 3, 4, 39)]
aNATMIArtery <- aNATMISubset[order(aNATMISubset$Target.cluster, aNATMISubset$Ligand.symbol),]
aNATMISubset <- subset(NATMImockvligated, Sending.cluster == "Capillary" & Ligand.symbol == ligand & Target.cluster == target)[,c(1, 2, 3, 4, 39)]
aNATMICapillary <- aNATMISubset[order(aNATMISubset$Target.cluster, aNATMISubset$Ligand.symbol),]
aNATMISubset <- subset(NATMImockvligated, Sending.cluster == "Lymphatic" & Ligand.symbol == ligand & Target.cluster == target)[,c(1, 2, 3, 4, 39)]
aNATMILymph <- aNATMISubset[order(aNATMISubset$Target.cluster, aNATMISubset$Ligand.symbol),]
aNATMISubset <- subset(NATMImockvligated, Sending.cluster == "Vein" & Ligand.symbol == ligand & Target.cluster == target)[,c(1, 2, 3, 4, 39)]
